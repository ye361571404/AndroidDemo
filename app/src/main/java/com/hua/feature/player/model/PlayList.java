package com.hua.feature.player.model;
 ...
 * Desc: PlayList
 */
public class PlayList<T extends BaseAudio> implements Parcelable {
    ...
    private List<AudioDecorator<T>> songs = new ArrayList<>();
    ...
    public PlayList(AudioDecorator<T> song) {
        songs.add(song);
        numOfSongs = 1;
    }
    ...
    @NonNull
    public List<AudioDecorator<T>> getSongs() {
        if (songs == null) {
            songs = new ArrayList<>();
        }
        return songs;
    ...
    public void setSongs(@Nullable List<AudioDecorator<T>> songs) {
        if (songs == null) {
            songs = new ArrayList<>();
        }
        this.songs = songs;
    ...
    public void addSong(@Nullable AudioDecorator<T> song) {
        if (song == null) return;
    ...
    public void addSong(@Nullable AudioDecorator<T> song, int index) {
        if (song == null) return;
    ...
    public void addSong(@Nullable List<AudioDecorator<T>> songs, int index) {
    ...
    public boolean removeSong(Song song) {
        ...
        if ((index = songs.indexOf(song)) != -1) {
            ...
            for (Iterator<AudioDecorator<T>> iterator = songs.iterator(); iterator.hasNext(); ) {
                AudioDecorator item = iterator.next();
                if (song.getPath().equals(item.getAudio().getAudioPath())) {
                    iterator.remove();
                    numOfSongs = songs.size();
                    return true;
                }
     ...
     */
    public AudioDecorator getCurrentSong() {
        if (playingIndex != NO_POSITION) {
    ...
    protected PlayList(Parcel in) {
        ...
        this.songs = new ArrayList<AudioDecorator<T>>();
        in.readList(this.songs, AudioDecorator.class.getClassLoader());
