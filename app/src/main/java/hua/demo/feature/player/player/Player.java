...
import hua.demo.feature.player.model.BaseAudio;
...
public class Player implements IPlayback, MediaPlayer.OnCompletionListener {
    ...
    public boolean play() {
        ...
        if (mPlayList.prepare()) {
            ...
            try {
                ...
                // audioPath = "/storage/emulated/0/xiaoma/universalToefl/audio/audioCourse/dongwu5.mp3";
    ...
    private void notifyPlayLast(AudioDecorator<? extends BaseAudio> song) {
        for (Callback callback : mCallbacks) {
            callback.onSwitchLast(song);
        }
    }

    private void notifyPlayNext(AudioDecorator<? extends BaseAudio> song) {
        for (Callback callback : mCallbacks) {
